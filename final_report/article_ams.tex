...
networks from French literary texts. The system was developed iteratively:
Named Entity Recognition went from a slow multi-model ensemble (spaCy + Stanza
+ Flair) to a fast rule-based backend completing all 37 chapters in $\sim$4
seconds; relation typing went from NLI-based classifiers (6--8 hours on CPU)
through LLM APIs (quota exhausted) to a sentiment-based approach using
\texttt{cardiffnlp/twitter-xlm-roberta-base-sentiment}, which processes the
full corpus in $\sim$8 minutes with no annotated data.
...
and produces one typed GraphML file per chapter for a Kaggle competition.
The actual label distribution across 37 chapters is hostile-dominant
(hostile~=~239, neutral~=~82, friendly~=~58), consistent with the political
intrigue and detective antagonism of the corpus.
We document each approach tried, its failure mode, and the algorithmic
errors that remain.
\end{abstract}
...
Both novels pose specific NLP challenges: invented proper nouns
(\textit{Trantor}, \textit{Terminus}) that NER models tag as persons,
robot names following the pattern \textit{R.~Daneel Olivaw}, and Asimov's
...
\texttt{Stanza} (French model), and optionally \texttt{Flair} (\texttt{fr-ner},
BiLSTM-CRF). A majority vote retained entities tagged as \texttt{PER} by
...
nouns (\textit{l'Empire}) as \texttt{PER} when appearing in subject position,
so majority vote did not filter them.

\textbf{Final approach --- rule-based NER} (\texttt{nlp\_manual\_ner.py}).
Four strategies are combined in priority order:

\begin{enumerate}
  \item \textit{Gazetteer lookup}: a curated dictionary of 49 characters
        and 47 locations from both novels, applied via longest-match
...
This variant processes all 37 chapters in $\approx$4 seconds with no
...
Pairs with weight~$<$~\texttt{min\_occurrences}~=~2 are discarded.

\subsection{Relation typing: four approaches, one retained}

\textbf{Approach 1 --- NLI with mDeBERTa} (\texttt{MoritzLaurer/\\
...
(\texttt{cardiffnlp/twitter-xlm-roberta-base-sentiment})~\cite{Barbieri2020}.
...
(each weighted by $|s|$).
If the normalised winner score falls below
\texttt{CONFIDENCE\_THRESHOLD}~=~0.45,
the label defaults to \textit{neutral}.
...
failure. \textit{Les Cavernes d'Acier} contributes the majority of hostile
edges through its detective-antagonist structure (Baley opposing robot
factions, serial confrontations). \textit{Prélude à Fondation} contributes
...
are few (Seldon/Dors, Baley/Daneel) and their positive valence must survive
...
The Twitter-XLM-RoBERTa model was trained on social-media text.
...
\textbf{Confidence threshold not optimised.}
\texttt{CONFIDENCE\_THRESHOLD}~=~0.45 was set empirically.
Without a held-out gold-standard annotation, there is no principled
basis for choosing this value. A lower threshold increases \textit{friendly}
and \textit{hostile} recall at the cost of precision; a higher threshold
increases precision but collapses borderline pairs to \textit{neutral}.
The \texttt{print\_validation\_report} utility is the only quality
signal currently available.

\textbf{Co-occurrence window over-linking.} A 150-token window can span paragraph boundaries. When characters A, B, and C share a scene, all three pairs receive co-occurrence edges even if only A and B interact directly. The relation typing module then classifies a pair with no real interaction; these typically fall below the confidence threshold and default to \textit{neutral}, but they inflate edge counts and occasionally produce spurious labels from ambient narrative tone.
...
(iii)~the confidence threshold was not tuned on held-out data.
